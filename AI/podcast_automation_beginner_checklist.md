...
- [x] (신규) Docker compose — Automation Service(18800)와 웹 대시보드(18080)를 한 번에 빌드·실행하는 스택 구성
- [ ] 6-4. 스케줄 자동 실행·UI 고도화
  - [x] 스케줄 DB 스키마 및 API 확장 — 요일/시간/플레이리스트 URL/Castopod UUID/활성 여부를 저장하고 CRUD 엔드포인트를 제공
  - [x] 백그라운드 스케줄러 구현 — FastAPI lifespan에서 주기적으로 현재 요일·시간과 일치하는 스케줄을 찾아 파이프라인을 실행
- [x] 스케줄 전용 대시보드 섹션 — 플레이리스트 카드에서 스케줄을 직접 추가/관리하는 UI 적용
- [x] 카드 내 스케줄 수정/삭제 UX — 기존 스케줄을 모달에서 편집하고 즉시 삭제할 수 있도록 인라인 액션을 제공
- [x] 스케줄 → 큐 자동 연동 — 스케줄 발동 시 해당 플레이리스트 Job을 큐에 추가하고 `pipeline-run`을 호출해 Castopod 업로드까지 진행
- [x] 큐 러너 — 큐에 `queued` 작업이 남아 있고 파이프라인이 비어 있으면 자동으로 `pipeline-run`을 재실행해 순차 처리
- [x] 큐 전체 삭제 — `/jobs/` 삭제 엔드포인트와 UI 버튼으로 정체된 작업을 한 번에 정리
  - [ ] 작업 생성 폼 개선 — “전체 다운로드” vs “선택 다운로드(ID 목록 또는 체크박스)” 옵션을 제공하고 파이프라인에 전달
  - [ ] 스케줄 실행 시 Castopod + YouTube 메타데이터 비교 결과를 로그/README에 기록
- [ ] Castopod 연동
